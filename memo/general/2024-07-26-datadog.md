...
# Monitor
## `query alert` and `metric alert`
- Only `query alert` is mentioned on [API document](https://docs.datadoghq.com/api/latest/monitors/#monitor-types)
- In terraform datadog provider, `query alert` and `metric alert` are interchangeable ([code](https://github.com/DataDog/terraform-provider-datadog/blob/07b8132c313757d07b3d7082907c5118fe1f55d2/datadog/resource_datadog_monitor.go#L79-L80))
